...
static INSTANCE: LazyLock<Mutex<UniformCameraSpeed>> = LazyLock::new(|| {
    let game_module = libmem::find_module("ShadowOfMordor.exe").unwrap();
    let target_address = unsafe {
        libmem::sig_scan("F3 0F 10 0D ? ? ? ? F3 0F 10 15 ? ? ? ? EB ? F3 0F 10 54 24", game_module.base, game_module.size)
...
impl Patch for UniformCameraSpeed {
    ...
    fn apply(&mut self) -> Result<(), String> {
        let patch_bytes: [u8; 8] = [
            0xF3, 0x0F, 0x10, 0x0D, 0x4C, 0xC1, 0x29, 0x01 // movss xmm1, dword ptr cs:const_flt_105
        ];
