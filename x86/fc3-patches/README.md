...
Quality-of-life patches for the game "Far Cry 3".
...
1. Download and extract the latest mod files (`fc3-patches-aio.zip`).
...
1. Navigate to your game binary folder (`Far Cry 3/bin/`).
2. Delete the `dsound.dll` file and the `plugins` folder (or just the specific `fc3_patches` files inside it if you have other plugins).

## Configuration

You can configure the mod by placing a `fc3_patches.toml` file next to `fc3_patches.asi` in the `plugins` folder.
An example configuration file can be found [here](./config/fc3_patches.toml).
